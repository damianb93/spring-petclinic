...
import com.github.damianb93.springpetclinic.repositories.PetTypeRepository;
...
class PetTypeServiceSDJpaImplTest {
    ...
    @InjectMocks
    private PetTypeServiceSDJpaImpl service;

    private PetType returnObj;

    @BeforeEach
    void setUp() {
        returnObj = PetType.builder().id(OBJ_ID).build();
    }

    @Test
    void findAll() {
        Set<PetType> returnSet = new HashSet<>();
        returnSet.add(PetType.builder().id(1L).build());
        returnSet.add(PetType.builder().id(2L).build());

        when(repository.findAll()).thenReturn(returnSet);

        Set<PetType> set = service.findAll();
    ...
    void findById() {
        ...
        PetType obj = service.findById(OBJ_ID);
    ...
    @Test
    void save() {
        PetType objToSave = PetType.builder().id(OBJ_ID).build();

        when(repository.save(any())).thenReturn(objToSave);

        PetType savedObj = service.save(objToSave);
